...
public:
    ...
    {
        ParametricInterval interval = { ivec2(100, 15), vec2(TwoPi, TwoPi), vec2(6, 0.5) * 2};
...
public:
    Quad(float w, float h) : m_size(vec2(w, h))
    {
...
public:
    ...
    {
        ParametricInterval interval = { ivec2(20, 30), vec2(TwoPi, TwoPi), vec2(15, 50) };
        SetInterval(interval);
    }
    vec3 Evaluate(const vec2& domain) const
    {
